...
normDb.settings.get("userParams").then(function (p) {
    if (!p) return;
    ...
    if (p.gender) {
        ...
        document.querySelectorAll("#gender-group .toggle-btn").forEach(function (b) {
            b.classList.toggle("active", b.dataset.gender === p.gender);
        });
    }
    if (p.goal) {
        selectedGoal = p.goal;
        document.querySelectorAll("#goal-group .goal-btn").forEach(function (b) {
            b.classList.toggle("active", b.dataset.goal === p.goal);
        });
    }
}).catch(function () { });
