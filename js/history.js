...
function renderHistory(records, from, to) {
    ...
    records.forEach(function(r) {
        ...
        if (isToday) {
            ...
            tagHtml = "<span class='day-tag tag-red'>+" + Math.abs(diff) + " ккал</span>";
        } else if (r.normReached) {
            tagHtml = "<span class='day-tag tag-green'>В норме ✓</span>";
        } else if (diff < 0) {
            tagHtml = "<span class='day-tag tag-yellow'>" + diff + " ккал</span>";
        } else {
            tagHtml = "<span class='day-tag tag-yellow'>+" + diff + " ккал (погрешность)</span>";
        }

        let parts = r.date.split("-");
        let d = new Date(Number(parts[0]), Number(parts[1])-1, Number(parts[2]));
        let barColor = isOver ? "background:#E53935" : (r.normReached ? "background:#2E7D52" : "");
